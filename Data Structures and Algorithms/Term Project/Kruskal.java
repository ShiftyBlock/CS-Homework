...
public class Kruskal {
  ...
  public static WUGraph minSpanTree(WUGraph g){
    ...
    HashTableChained hashV = new HashTableChained(g.vertexCount());
    ...
    for(Object vertex : g.getVertices()){
      ...
      hashV.insert(vertex, vertMap++);
      neighbors = g.getNeighbors(vertex);
      for(int i = 0; i<neighbors.neighborList.length; i++){
        edges[eIdx][0] =neighbors.weightList[i];
        edges[eIdx][1]= vertex;
        edges[eIdx][2] =neighbors.neighborList[i];
        eIdx++;
      }
    }
    Object gEdges[][] = new Object[eIdx][3];
    for(int i = 0; i<gEdges.length; i++){
    	gEdges[i]=edges[i];
    	}
    edges = mergesort(gEdges);    
    int vert1;
    int vert2;
    for (Object[] edge : edges){
      vert1 = (Integer)hashV.find(edge[1]).value();
      vert2 = (Integer)hashV.find(edge[2]).value();
