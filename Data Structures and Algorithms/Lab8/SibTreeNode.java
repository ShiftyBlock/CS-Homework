...
class SibTreeNode extends TreeNode {
    ...
    public void insertChild(Object item, int c) throws InvalidNodeException {
        ...
        child.parent = this;
        SibTreeNode current = firstChild;
        if (c <= 1 || firstChild == null) {
            firstChild = child;
            child.nextSibling = current;
        } else {
            int i = 2;
            SibTreeNode next = firstChild.nextSibling;
            while (i < c && next != null) {
    ...
    public void removeLeaf() throws InvalidNodeException {
        ...
        if (children() == 0) { //  remove if the node is leaf
            if (this == myTree.root) {
                myTree.root = null;
            } else {
                SibTreeNode p = parent;
                if (p.firstChild == this) {
                    p.firstChild = nextSibling;
                } else {
                    SibTreeNode c = p.firstChild;
                    while (c.nextSibling != this) {
                        // loop until finding myPrevSib
                        c = c.nextSibling;
                    }
