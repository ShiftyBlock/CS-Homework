...
public class Set {
  ...
  public void union(Set s) {
    ListNode bob1 = this.store.front();
    ListNode bob2 = s.store.front();
    ListNode end1 = this.store.back();
    ListNode end2 = s.store.back();

    try {
        for (int i = 0; i < s.cardinality(); i++) {
            if (((Comparable)bob1.item()).compareTo((Comparable)bob2.item()) < 0) {
                bob1 = bob1.next();
                i--;
            } else if (((Comparable)bob1.item()).compareTo((Comparable)bob2.item()) > 0) {
                bob1.insertBefore(bob2.item());
                bob2 = bob2.next();
            } 
            else if (((Comparable)bob1.item()).compareTo((Comparable)bob2.item()) == 0) bob2 = bob2.next();
            
        }
    } catch (InvalidNodeException e) {
        try {
            while (bob2 != end1) { 
                this.store.insertBack(bob2.item());
                bob2 = bob2.next();
            }
            this.store.insertBack(bob2.item());
